...
from benchmark.data_gen import generate_dummy_source_face, generate_dummy_video
...
class BenchmarkRunner:
    ...
    def track_resources(self, stop_event: threading.Event, stats: dict):
        ...
        while not stop_event.is_set():
            ...
            peak_ram = max(peak_ram, used_ram_mb)
            ...
            if has_gputil:
                ...
                if gpus:
                    ...
                    peak_vram = max(peak_vram, used_vram_mb)
    ...
    def run_benchmark(self, config: BenchmarkConfig, output_dir: Path) -> BenchmarkResult:
        ...
        total_frames = int(info.get("total_frames", config.frames) or config.frames)
        ...
        avg_id_score = 0.0
        id_report = identity_validator.generate_identity_report()
        if id_report:
            avg_id_score = id_report.average_similarity * 100.0

        avg_quality_score = 85.0
        if hasattr(self.swapper, "_app") and self.swapper._app is not None:
            assessor = FaceQualityAssessor(face_analysis_app=self.swapper._app)
            try:
                q_report = assessor.assess_image(str(img_path))
