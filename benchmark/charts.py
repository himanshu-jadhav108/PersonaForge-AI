...
def generate_charts(results: list[BenchmarkResult], output_dir: Path):
    ...
    # 2. FPS Comparison
    fig2 = px.bar(df, x="config_id", y="fps", color="config_id", title="FPS by Configuration (Higher is Better)")
    fig2.update_layout(template="plotly_dark")
    fig2.write_html(str(output_dir / "fps_comparison.html"))

    # 3. RAM vs VRAM Usage
    fig3 = px.bar(df, x="config_id", y=["peak_ram_mb", "peak_vram_mb"], barmode="group", title="Memory Usage (MB)")
