...
st.set_page_config(page_title="PersonaForge Analytics", page_icon="⚡", layout="wide", initial_sidebar_state="expanded")
...
page = st.sidebar.radio(
    "Navigation", ["Overview", "Performance", "Identity Metrics", "Benchmark Results", "System Health"]
)
...
if page == "Overview":
    ...
    if data and data.get("trends"):
        ...
        fig.update_layout(
            template="plotly_dark", plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", yaxis_range=[0, 100]
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
    ...
    benchmark_df = pd.DataFrame(
        {"Model": ["Ghost", "Inswapper", "SimSwap"], "Avg Quality": [92.5, 88.0, 81.2], "Avg FPS": [24.5, 30.1, 15.0]}
    )
