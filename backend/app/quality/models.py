...
class QualityMetrics(BaseModel):
    ...
    face_size: float = Field(
        ..., description="Face size score (0-100). Higher means face takes up a good portion of the image."
    )
...
class QualityReport(BaseModel):
    ...
    recommendations: list[str] = Field(
        default_factory=list, description="Actionable recommendations based on the scores."
    )
...
class TemporalStabilityMetrics(BaseModel):
    ...
    stability_score: float = Field(
        100.0, description="Normalized temporal stability score (0-100). Higher is more stable."
    )
